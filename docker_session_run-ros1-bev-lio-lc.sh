...
IMAGE_NAME='bev-lio-lc'
TMUX_SESSION='ros1_session'

DATASET_CONTAINER_PATH='/workspace/benchmark/dataset/recorded-bev-lio-lc.bag'
BAG_OUTPUT_CONTAINER='/workspace/benchmark/recordings'

RECORDED_BAG_NAME="recorded-bev-lio-lc.bag"
...
echo "=== bev-lio-lc rosbag pipeline ==="
...
source /workspace/benchmark/devel/setup.bash
roslaunch bev_lio_lc mapping_velodyne.launch use_sim_time:=true
'\'' C-m
...
docker run -it --rm \
  ...
  /bin/bash -c "
    ...
    source /opt/ros/noetic/setup.bash
    source /workspace/benchmark/devel/setup.bash
    rosrun bev-lio-lc-to-hdmapping listener  \
      \"$BAG_OUTPUT_CONTAINER/$RECORDED_BAG_NAME\" \
      \"$BAG_OUTPUT_CONTAINER/$HDMAPPING_OUT_NAME-bev-lio-lc \"
  "
